...
def test_representative_cases(tmp_path: Path):
    ...
    header = (
        "q1_rad,q2_rad,q3_rad,left_tilt_servo_deg,right_lift_servo_deg,"
        "gripper_servo_deg,safe,connected,reason,contacts\n"
    )
    lines = [header]
    for q1 in (-0.2, -0.1, 0.0, 0.1, 0.2):
        for q2 in (-0.2, -0.1, 0.0, 0.1, 0.2):
            for q3 in (0.0, 0.1, 0.2, 0.3, 0.4):
                lines.append(f"{q1},{q2},{q3},90,90,0,1,1,safe,\n")
    connected.write_text("".join(lines), encoding="utf-8")
    all_samples.write_text(
        "".join(lines)
        + "0.3,0.3,0.4,90,90,180,0,0,collision,left|right\n",
        encoding="utf-8",
    )
