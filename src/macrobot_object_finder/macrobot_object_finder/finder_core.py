...
def temporal_to_result_payload(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "localization": {
            "available": bool(str(getattr(message, "localization_method", "")).strip()),
            "method": str(getattr(message, "localization_method", "")),
            "quality": float(getattr(message, "localization_quality", 0.0)),
        },
        "orientation": {
            "angle_deg": float(getattr(message, "orientation_deg", 0.0)),
            "class": str(getattr(message, "orientation_class", "unknown")),
            "quality": float(getattr(message, "orientation_quality", 0.0)),
        },
    }
    if roi is not None:
        payload["bbox"] = {
