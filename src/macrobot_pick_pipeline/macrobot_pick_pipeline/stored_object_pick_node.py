...
class StoredObjectPickNode(Node):
    ...
    def _declare_parameters(self) -> None:
        defaults: Dict[str, Any] = {
            ...
            "approach_max_turn_step_deg": 4.0,
    ...
    def _detection_callback(self, msg: String) -> None:
        ...
        try:
            ...
            source_stamp = float(payload.get("stamp_sec", now_sec))
            published_stamp = float(payload.get("published_at_sec", now_sec))
        ...
        if self.phase == "record_wait_detection":
            ...
            if stable is not None:
                ...
                if now - self.last_stability_diagnostic_monotonic >= 1.0:
                    ...
                    self._publish_stability_diagnostic()
            return
    ...
    # Misc helpers
    # ------------------------------------------------------------------
    def _current_record_alignment(self) -> AlignmentProfile:
        """Return the current gating profile used for record-search.

        ``default_alignment`` is constructed once during node startup.  During
        commissioning we often relax stability/localization parameters with
        ``ros2 param set`` while the node is running.  For record-search and
        other record modes there is no stored profile yet, so read these gate
        parameters live instead of using the stale startup snapshot.
        """
        ...
        try:
            alignment = AlignmentProfile(
                ...
                object_name=self.object_name or "default",
                pick_profile=self.record_pick_profile or self.object_name or "default",
                reference_point_base=self.default_alignment.reference_point_base,
                recorded_at="",
                frame_id=str(self.get_parameter("base_frame").value),
                minimum_score=float(self.get_parameter("minimum_score").value),
                stability_count=int(self.get_parameter("stability_count").value),
    ...
    def _publish_stability_diagnostic(self) -> None:
        alignment = self._current_record_alignment()
        now_wall = time.time()
        cutoff = now_wall - alignment.stability_window_sec
        samples = list(self.filter.samples)
        recent = [item for item in samples if item.stamp_sec >= cutoff]
        target_key = self.object_name.strip().casefold()
        same_object = [
        ...
        fail_counts: Dict[str, int] = {
            ...
            "localization_quality_below_minimum": 0,
            "depth_std_above_maximum": 0,
            "center_std_above_maximum": 0,
            "orientation_mismatch_or_low_quality": 0,
        }
        eligible: List[DetectionSample] = []
        ...
        for item in same_object:
            rejected = False
            if item.score < alignment.minimum_score:
                fail_counts["score_below_minimum"] += 1
                rejected = True
            if item.localization_quality < alignment.minimum_localization_quality:
                fail_counts["localization_quality_below_minimum"] += 1
                rejected = True
            if item.depth_std_m > alignment.maximum_depth_std_m:
                fail_counts["depth_std_above_maximum"] += 1
                rejected = True
            if item.center_std_px > alignment.maximum_center_std_px:
                fail_counts["center_std_above_maximum"] += 1
                rejected = True
            if required_orientation and (
                item.orientation_class.strip().casefold() != required_orientation
                or item.orientation_quality < alignment.minimum_orientation_quality
            ):
                fail_counts["orientation_mismatch_or_low_quality"] += 1
                rejected = True
            if not rejected:
                eligible.append(item)

        cluster_radius = None
        if len(eligible) >= alignment.stability_count:
            latest = eligible[-alignment.stability_count:]
            coordinates = list(zip(*(item.point_base for item in latest)))
            med = tuple(float(sorted(axis)[len(axis) // 2]) for axis in coordinates)
            cluster_radius = max(math.dist(item.point_base, med) for item in latest)

        latest_payload: Dict[str, Any] = {}
        if same_object:
            latest = same_object[-1]
            latest_payload = {
                "score": latest.score,
                "point_base": list(latest.point_base),
                "localization_quality": latest.localization_quality,
                "depth_std_m": latest.depth_std_m,
                "center_std_px": latest.center_std_px,
                "orientation_class": latest.orientation_class,
                "orientation_quality": latest.orientation_quality,
                "age_sec": max(0.0, now_wall - latest.stamp_sec),
            }

        self._publish_status(
            "record_stability_waiting",
            phase=self.phase,
            total_buffered_samples=len(samples),
            recent_samples=len(recent),
            recent_target_samples=len(same_object),
            eligible_samples=len(eligible),
            required_samples=alignment.stability_count,
            window_sec=alignment.stability_window_sec,
            cluster_radius_m=cluster_radius,
            required_radius_m=alignment.stability_radius_m,
            gates={
                "minimum_score": alignment.minimum_score,
                "minimum_localization_quality": alignment.minimum_localization_quality,
                "maximum_depth_std_m": alignment.maximum_depth_std_m,
                "maximum_center_std_px": alignment.maximum_center_std_px,
                "require_orientation_match": alignment.require_orientation_match,
                "minimum_orientation_quality": alignment.minimum_orientation_quality,
            },
            reject_counts=fail_counts,
