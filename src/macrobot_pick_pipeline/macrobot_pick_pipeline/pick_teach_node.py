...
class PickTeachNode(Node):
    ...
    def __init__(self) -> None:
        super().__init__("macrobot_pick_teach")
    ...
    def _action_record_primitive(
        ...
        speed_scale = max(0.05, min(1.0, float(payload.get("speed_scale", 1.0))))
        section = self.report.section("primitives")
        primitives = dict(section.get("primitives", {})) if isinstance(section.get("primitives"), Mapping) else {}
        camera_context = self._camera_context()
        primitives[name] = {
            "target_q": list(self.current_q),
            "speed_scale": speed_scale,
            "operator_pass": bool(payload.get("approved", True)),
            "notes": str(payload.get("notes", "")),
            "recorded_at": utc_now(),
            "recorded_by": "camera_arm_teach",
        ...
        self._result(command_id, "primitive_recorded", name=name, primitive=primitives[name])
    ...
    def _action_test_primitive(
        ...
        if not isinstance(primitives, Mapping) or name not in primitives:
            raise ValueError(f"primitive not found: {name}")
        raw = primitives[name]
        if not isinstance(raw, Mapping):
            raise ValueError(f"invalid primitive: {name}")
        q = q3(raw.get("target_q"), name=f"primitive.{name}.target_q")
        self._start_motion(command_id, q, f"primitive:{name}")
