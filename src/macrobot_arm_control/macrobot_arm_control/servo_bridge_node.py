...
class ArmServoBridgeNode(Node):
    ...
    def _send_servo_set(self, q: Q, force: bool = False) -> None:
        ...
        for key, axis in axes:
            value = pulses[key] if self.command_mode == "pulse_us" else commands_deg[key]
            previous = self.last_commands.get(key)
            if not force and previous is not None and abs(value - previous) < self.minimum_command_delta:
                continue
            if self.command_mode == "pulse_us":
                command = f"SERVO_US {axis.channel} {value:.1f}"
            else:
                command = f"SERVO {axis.channel} {value:.3f}"
            preview = String()
    ...
    def _disable_callback(self, _: Empty) -> None:
        self.active = False
        for axis in (self.mapping.lift, self.mapping.tilt, self.mapping.gripper):
            command = f"SERVO_OFF {axis.channel}"
            preview = String()
            preview.data = command
            self.preview_pub.publish(preview)
            if not self.dry_run:
