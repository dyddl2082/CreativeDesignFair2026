    ...
    for check_index in range(6):
        ...
        if object_state.state == ObjectState.VISIBLE:
            ...
            if pick_result.state != ActionState.SUCCEEDED:
                return TaskOutcome(
                    ...
                    data={"checked_angle_deg": check_index * 60},
                )

            return TaskOutcome(
                status=TaskStatus.SUCCEEDED,
                message="고무를 찾아 집었습니다.",
                data={"found_angle_deg": check_index * 60},
            )
        ...
        if object_state.state != ObjectState.NOT_VISIBLE:
            if check_index == 0:
                return TaskOutcome(
                    ...
                    message=object_state.error_message or "고무의 가시 상태를 확인할 수 없습니다.",
                )

            return TaskOutcome(
                status=TaskStatus.PARTIALLY_SUCCEEDED,
                message=object_state.error_message or "회전 탐색 중 고무의 가시 상태를 확인할 수 없습니다.",
                data={"completed_checks": check_index},
            )
        ...
        if turn_result.state != ActionState.SUCCEEDED:
            return TaskOutcome(
                ...
                data={
                    "completed_checks": check_index + 1,
                    "completed_turns": check_index,
                },
            )

    return TaskOutcome(
        status=TaskStatus.FAILED,
        message="6개 방향을 모두 확인했지만 고무를 찾지 못했습니다.",
        data={
            "completed_checks": 6,
            "completed_turns": 6,
        },
